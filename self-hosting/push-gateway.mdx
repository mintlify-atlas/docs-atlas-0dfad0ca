...
description: "Deploy and operate buzz-push-gateway, the standalone APNs last-hop for iOS push notifications. Covers ports, secrets, Helm, Prometheus metrics, and key rotation."
---
...
All 14 required variables must be set for the gateway to start. Readiness rejects a partial or misconfigured schema.
...
|-------|-----------|---------|----------------|
| `PushGatewayConfigurationFault` | Any `configuration_fault` outcomes for 10 min | critical | APNs provider token/topic is unhealthy. Check `.p8` key, `BUZZ_PUSH_APNS_KEY_ID`, `TEAM_ID`, and `TOPIC`. Nothing is delivering. |
